...
public class Properties extends ConcurrentHashMap<String, Integer> {
	...
	/**
	 * 指定した{@link java.io.File File}オブジェクトに対してこのプロパティが保持しているキー及び値の組み合わせを暗号化した上で保存します。
	 * 
	 * @param file
	 *            保存先のファイル
	 * @throws Exception
	...
	/**
	 * 指定した{@link java.io.OutputStream OutputStream}オブジェクトに対してこのプロパティが保持しているキー及び値の組み合わせを保存します。<br>
	 * このメソッドではストリームを閉じません。
	 * 
	 * @param os
	 *            保存先のストリーム
